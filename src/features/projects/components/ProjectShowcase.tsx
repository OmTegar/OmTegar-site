...
import { getTechIcon } from "../config/icons";
    ...
    return (
        ...
        >
          ...
          // exploring the digital frontier through code and architecture.
            ...
            <FilterDock currentFilter={filter} onFilterChange={setFilter} />
...
// Sub-components to enforce SRP (Splitting big components)

const FilterDock = ({ currentFilter, onFilterChange }: { currentFilter: string, onFilterChange: (f: string) => void }) => {
    const filters = [
        ...
        <div className="flex flex-wrap gap-3 items-center">
            {filters.map((item) => (
                <button
                    ...
                    className={`px-6 py-2 rounded-full text-xs font-bold tracking-widest uppercase transition-all border ${currentFilter === item.id
...
const Card = ({ data }: { data: Project }) => {
    ...
    return (
        <>
            ...
            <ProjectModal isOpen={isOpen} setIsOpen={setIsOpen} data={data} />
        </>
    );
};

const ProjectModal = ({ isOpen, setIsOpen, data }: { isOpen: boolean, setIsOpen: (v: boolean) => void, data: Project }) => {
    return (
